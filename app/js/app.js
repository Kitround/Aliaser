...
async function createAlias(aliasName,accountId,note=''){
  ...
  try{
    ...
    else if(acc.provider==='haltman') newAlias=await haltmanCreateAlias(acc,resolvedName);
...
async function disableAlias(alias){
  ...
  try{
    ...
    }else if(alias.provider==='haltman'){
      await haltmanDeleteAlias(alias);
    }else{
      await ovhDeleteAlias(alias,acc);
    }
    // Only OVH/IK/Haltman store disabled state locally
...
async function enableAlias(alias){
  ...
  try{
    if(alias.provider==='simplelogin'){
      ...
      const newAlias=acc.provider==='infomaniak'?await ikCreateAlias(acc,aliasName):acc.provider==='haltman'?await haltmanCreateAlias(acc,aliasName):await ovhCreateAlias(acc,aliasName);
      state.disabledAliases=state.disabledAliases.filter(d=>!(d.aliasAddress===alias.aliasAddress&&d.accountId===alias.accountId));
      const i=state.aliases.findIndex(a=>a.aliasAddress===alias.aliasAddress&&a.accountId===alias.accountId);
...
function renderList(){
  ...
  state.filteredAliases.forEach(a=>{
    ...
    const providerClass=
      ...
      a.provider==='cloudflare'?'cf':
      a.provider==='haltman'?'haltman':'ovh';
    const providerLabel=
      a.provider==='infomaniak'?'Infomaniak':
      a.provider==='simplelogin'?'SimpleLogin':
      a.provider==='addy'?'Addy':
      a.provider==='cloudflare'?'Cloudflare':
      a.provider==='haltman'?'Haltman':'OVH';
    html+=`
...
function renderAccountList(){
  ...
  el.innerHTML=state.accounts.map(acc=>{
    const cls=
      ...
      acc.provider==='cloudflare'?'cf':
      acc.provider==='haltman'?'haltman':'ovh';
    const lbl=
      acc.provider==='infomaniak'?'Infomaniak':
      acc.provider==='simplelogin'?'SimpleLogin':
      acc.provider==='addy'?'Addy':
      acc.provider==='cloudflare'?'Cloudflare':
      acc.provider==='haltman'?'Haltman':'OVH';
...
function renderAccountSelector(){
  ...
  pillsEl.innerHTML=state.accounts.map(acc=>{
    const domain=
      ...
      acc.provider==='haltman'?(acc.domain||'haltman'):
...
function render(){
  ...
  const countEl=document.getElementById('topbar-alias-count');
  if(countEl){
...
let _pillTimer=null;
function showCreatedAliasPill(aliasAddress){
  const pill=document.getElementById('created-alias-pill');
  const txt=document.getElementById('created-alias-pill-text');
  if(!pill||!txt)return;
  txt.textContent=aliasAddress;
  // Re-trigger animation
  pill.classList.remove('visible');
  pill.offsetWidth;
  pill.classList.add('visible');
  clearTimeout(_pillTimer);
  _pillTimer=setTimeout(()=>pill.classList.remove('visible'),6000);
  pill.onclick=()=>scrollToAndHighlightAlias(aliasAddress);
}

function scrollToAndHighlightAlias(aliasAddress){
  const alias=state.aliases.find(a=>a.aliasAddress===aliasAddress);
  if(!alias)return;
  const card=document.querySelector(`.alias-card[data-id="${CSS.escape(alias.id)}"]`);
  if(!card)return;
  card.scrollIntoView({behavior:'smooth',block:'center'});
  card.classList.remove('alias-card-new');
  card.offsetWidth;
...
const _addAccountTitles={
  ...
  'section-add-haltman':'New Haltman account',
...
// Haltman
document.getElementById('btn-add-haltman-account').addEventListener('click',async()=>{
  showAddForm('section-add-haltman');
  document.getElementById('s-haltman-token').value='';
  document.getElementById('s-haltman-label').value='';
  const sel=document.getElementById('s-haltman-domain');
  sel.innerHTML='<option value="">Loading…</option>';
  try{
    const domains=await proxyCall('haltman','GET','/api/domains',null,{token:''});
    sel.innerHTML=(Array.isArray(domains)?domains:[]).map(d=>`<option value="${esc(d)}">${esc(d)}</option>`).join('');
  }catch(e){
    sel.innerHTML='<option value="">Could not load domains</option>';
  }
});
document.getElementById('btn-cancel-add-haltman').addEventListener('click',hideAddForms);
document.getElementById('btn-save-haltman-account').addEventListener('click',async()=>{
  const token=document.getElementById('s-haltman-token').value.trim();
  const domain=document.getElementById('s-haltman-domain').value.trim();
  const labelInput=document.getElementById('s-haltman-label').value.trim();
  if(!token){showError('Please enter your Haltman API key');return;}
  if(!domain){showError('Please select a domain');return;}
  const btn=document.getElementById('btn-save-haltman-account');
  btn.disabled=true;btn.textContent='Verifying…';
  const tempAcc={token};
  try{
    await proxyCall('haltman','GET','/api/alias/list?limit=1',null,{token});
    const label=labelInput||domain;
    state.accounts.push({id:genId(),provider:'haltman',label,domain,token});
    saveServerState();saveAccountCredentials();hideAddForms();
  }catch(e){
    showError('Haltman error: '+e.message);
  }finally{
    btn.disabled=false;btn.textContent='Save account';
  }
});
...
document.getElementById('btn-create-alias').addEventListener('click',async()=>{
  ...
  try{
    ...
    if(newAlias?.aliasAddress){
      copyText(newAlias.aliasAddress);
      showCreatedAliasPill(newAlias.aliasAddress);
      setTimeout(()=>scrollToAndHighlightAlias(newAlias.aliasAddress),80);
    }
    hideError();
